        ...
        request(options, function(error, response, body) {
            if (error) {
                ...
                let commits = JSON.parse(body)
                    ...
                    .filter(item => moment(item.created_at).format("YYYY-MM-DD") === moment().format("YYYY-MM-DD"));
                // commits 按照msg去重
    ...
    if (content === '%') {
        getCommits().then((commits) => {
            ...
            for (const cmt of commits) {
                public.addRes(result_array, cmt.msg, `${moment(cmt.created_at).format("HH:mm")}（${moment(cmt.created_at).fromNow()}）- Enter 写入日报`, cmt.msg);
            }
